...
function LinkedList(headNode) {
  ...
  this.append = function (value) {
    if (this.tailNode) {
      this.tailNode.nextNode = new Node(value, this.tailNode);
  ...
  this.prepend = function (value) {
    if (this.headNode) {
      const newHead = new Node(value, null, this.headNode);
  ...
  this.pop = function () {
    if (this.listSize === 1) {
      ...
      let newTail = this.tailNode.previousNode;
    ...
    if (index === 0) {
      ...
      while (current) {
        if (counter !== index) {
          ...
          current.previousNode.nextNode = new Node(
            value,
            current.previousNode,
            current.nextNode,
          );
          current.nextNode.previousNode = current.previousNode.nextNode;
          break;
        }
      }
    }
